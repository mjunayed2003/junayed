...
function InboxTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await client_api.get("/api/dashboard/message/read-all");
      if (res?.status === true) setData(res?.data);
    };
    fetchData();
  }, []);

  const DeleteMessage = async (id) => {
    const resAlert = await DeleteAlert(`/api/dashboard/message/delete?id=${id}`);
    if (resAlert) {
      const res = await client_api.get("/api/dashboard/message/read-all");
      if (res?.status === true) setData(res?.data);
    }
  ...
  return (
    <div className="rounded-xl bg-[#36404A] p-[30px] m-[30px]">
      <h2 className="text-slate-700 text-2xl font-semibold mb-2">All Message</h2>
      <DataTable
        fixedHeader
