...
function EducationTable() {
  ...
  useEffect(() => {
    client_api.get("/api/dashboard/education/read-all").then((res) => {
      if (res?.status === true) {
        setData(res?.data);
      }
    });
  ...
  const Delete = (id) => {
    DeleteAlert(`/api/dashboard/education/delete?id=${id}`).then(async (res) => {
      if (res) {
        await client_api.get("/api/dashboard/education/read-all").then((res) => {
          if (res?.status === true) {
            setData(res?.data);
          }
        });
      ...
      selector: (row) => (
        <div className="flex gap-2 item-center">
          <div className="p-2 cursor-pointer  ">
            <Link href={`/dashboard/education/edit-education/${row?.id}`}>
              <FaPenToSquare />
            </Link>
          </div>
          <div className="p-2 cursor-pointer    ">
