...
function ExperienceTable() {
  ...
  useEffect(() => {
    client_api.get("/api/dashboard/experience/read-all").then((res) => {
      if (res?.status === true) {
        setData(res.data);
      }
    });
  ...
  const Delete = (id) => {
    DeleteAlert(`/api/dashboard/experience/delete?id=${id}`).then(async (res) => {
      if (res) {
       await client_api.get("/api/dashboard/experience/read-all").then((res) => {
          if (res?.status === true) {
            setData(res?.data);
          }
        });
      ...
      selector: (row) => (
        <div className="flex gap-2 item-center">
          <div className="p-2 cursor-pointer	">
            <Link href={`/dashboard/experience/edit-experience/${row?.id}`}>
              <FaPenToSquare />
            </Link>
          </div>
          <div className="p-2 cursor-pointer	">
