...
const EditExperienceComponent = ({ id }) => {
  ...
  useEffect(() => {
    client_api.get(`/api/dashboard/experience/read-single?id=${id}`).then((res) => {
      if (res?.status === true) {
        setData(res?.data[0]);
  ...
  const formSubmit = async () => {
    ...
    client_api.update(`/api/dashboard/experience/update?id=${id}`, {
      title,
      company,
      time_section,
      long_des,
  ...
  return (
    <section>
      ...
      <div className="m-[30px] rounded-xl bg-[#36404A] p-[30px]">
        <h2 className="text-xl font-medium text-white">Update experience</h2>
